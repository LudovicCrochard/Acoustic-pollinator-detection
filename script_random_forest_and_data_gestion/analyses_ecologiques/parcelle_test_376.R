...
df376_pol <- filter(df376, score_pol>=73.8)
t <- unique(df376_pol$fichier)
df376_pol <- as.data.frame(unique(df376_pol$fichier)) #controle que bien une seule ligne par enregistrement
names(df376_pol) <- "Group.1"
df376_pol <- separate(data = df376_pol, col = Group.1, sep = "_", into = c("Field", "Parcelle", "Pied", "Date", "Heure", "ext"))

df376_pol$Date <- as.Date(df376_pol$Date, format = "%Y%m%d")
df376_pol$Nb_buzz <- 1
df376_pol <- df376_pol[,c(2,3,4,5,7)]
df376_pol$jour <- as.numeric(format(df376_pol$Date, "%j"))
df376_pol$semaine <- isoweek(df376_pol$Date)
...
col <- c("turquoise2", "violetred1", )
col <- sample(colors()[1:100], 24, replace = FALSE)

ggplot(data = test1, aes(x=num_Heure, y=Nb_buzz))+
  geom_boxplot(outlier.shape = NA)+
  geom_point(data=test1, aes(x=num_Heure, y=Nb_buzz,color=as.character(jour)))+ scale_color_manual(values = col)+
  geom_point(data=test,aes(x = num_Heure, y=Nb_buzz), col="red", size=3)+ theme_classic()+
  xlab("Heure")+ylab("Nombre de buzz")+ggtitle("Nombre de buzz moyen par heure au cours de la saison")


meteo <- fread("data/meteo_data.csv")
meteo_376 <- filter(meteo, id==376)
meteo_376 <- filter(meteo_376, day<=225)
meteo_376 <- filter(meteo_376, day>=203)

t376 <- unique(meteo_376[,c(1,2,4,5)])
